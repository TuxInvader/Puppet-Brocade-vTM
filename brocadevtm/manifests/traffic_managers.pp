...
# Type:array, Details:{"sysctl"=>{"description"=>"The name of the kernel parameter, e.g. net.ipv4.forward", "type"=>"string"}, "description"=>{"description"=>"Associated optional description for the sysctl", "type"=>"string"}, "value"=>{"description"=>"The value of the kernel parameter", "type"=>"string"}}
#
...
# How many worker threads the Aptimizer process should create to optimise content. By default, one thread will be created for each CPU on the system.
#
# [*basic__num_children*]
...
#
# [*basic__start_sysd*]
# Whether or not to start the sysd process on software installations. Appliance and EC2 will always run sysd regardless of this config key.
#
# [*basic__trafficip*]
# A table mapping interfaces to networks, used by the traffic manager to select which interface to raise a Traffic IP on.
# Type:array, Details:{"name"=>{"description"=>"A network interface.", "type"=>"string"}, "networks"=>{"description"=>"A set of IP/masks to which the network interface maps.", "type"=>"array", "uniqueItems"=>true, "items"=>{"type"=>"string"}}}
#
# [*basic__updaterIP*]
...
# Type:array, Details:{"name"=>{"description"=>"The name of a host.", "type"=>"string"}, "ip_address"=>{"description"=>"The static IP address of the host.", "type"=>"string"}}
#
# [*appliance__if*]
# A table of network interface specific settings.
# Type:array, Details:{"name"=>{"description"=>"A network interface name.", "type"=>"string"}, "autoneg"=>{"description"=>"Whether auto-negotiation should be enabled for the interface.", "type"=>"boolean"}, "bmode"=>{"description"=>"The trunking mode used for the interface (only 802.3ad is currently supported).", "type"=>"string", "enum"=>["802_3ad", "balance_alb"]}, "bond"=>{"description"=>"The trunk of which the interface should be a member.", "type"=>"string", "pattern"=>"^(bond\\d+)?$"}, "duplex"=>{"description"=>"Whether full-duplex should be enabled for the interface.", "type"=>"boolean"}, "mtu"=>{"description"=>"The maximum transmission unit (MTU) of the interface.", "type"=>"integer", "minimum"=>68, "maximum"=>9216}, "speed"=>{"description"=>"The speed of the interface.", "type"=>"string", "enum"=>["10", "100", "1000"]}}
#
# [*appliance__ip*]
# A table of network interfaces and their network settings.
# Type:array, Details:{"name"=>{"description"=>"A network interface name.", "type"=>"string"}, "addr"=>{"description"=>"The IP address for the interface.", "type"=>"string"}, "isexternal"=>{"description"=>"Whether the interface is externally facing.", "type"=>"boolean"}, "mask"=>{"description"=>"The IP mask (netmask) for the interface.", "type"=>"string"}}
#
...
# The IP addresses of the nameservers the appliance should use and place in "/etc/resolv.conf".
# Type:array, Details:
#
...
# Type:array, Details:{"name"=>{"description"=>"A destination IP address.", "type"=>"string"}, "gw"=>{"description"=>"The gateway IP to configure for the route.", "type"=>"string"}, "if"=>{"description"=>"The network interface to configure for the route.", "type"=>"string"}, "mask"=>{"description"=>"The netmask to apply to the IP address.", "type"=>"string"}}
#
...
#
# [*appliance__shim_ips*]
# The IP addresses of the Cloud Steelheads to use, as a space or comma separated list. If using priority load balancing this should be in ascending order of priority (highest priority last).
#
...
# The IP or hostname of the proxy server to use to connect to the portal. Leave blank to not use a proxy server.
#
# [*appliance__shim_proxy_port*]
# The port of the proxy server, must be set if a proxy server has been configured.
#
...
# This key overrides the product ID used by traffic manager instances to discover each other when clustering. Traffic managers will only discover each other if their product IDs are the same and their versions are compatible.
#
...
#
# [*ec2__instanceid*]
# The EC2 instance ID of this EC2 virtual appliance, should be set when the appliance is first booted. Not required for non-EC2 systems.
#
...
#
# [*ec2__vpcid*]
# The ID of the VPC the instance is in, should be set when the appliance is first booted. Not required for non-VPC EC2 or non-EC2 systems.
#
# [*fault_tolerance__ospfv2_ip*]
# The traffic manager's permanent IPv4 address which the routing software will use for peering and transit traffic, and as its OSPF router ID.
#  If set to empty, then the address used to communicate with the default IPv4 gateway is used instead.
#  Specifying 0.0.0.0 will stop the traffic manager routing software from running.
#
...
# The routing software log level. Values are: 0 - emergency 1 - alert 2 - critical 3 - error 4 - warning 5 - notification 6 - informational 7 - debug Messages with priority less or equal to the set level will be logged.
#
# [*java__port*]
...
# How many worker threads the Kerberos Protocol Transition helper process will use.
#
# [*rest_api__bind_ips*]
