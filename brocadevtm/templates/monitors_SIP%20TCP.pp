# === Define: brocadevtm::monitors_SIP%20TCP
#
...
define brocadevtm::monitors_SIP%20TCP (
  ...
  info ("Configuring monitors_SIP%20TCP ${name}")
  vtmrest { "monitors/SIP%20TCP/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/monitors_SIP%20TCP.erb'),
    internal => 'monitors_SIP%20TCP',
    debug => 0,
  }
}
