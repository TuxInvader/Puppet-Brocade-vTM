# === Define: brocadevtm::event_types_TrafficScript%20Error
#
...
define brocadevtm::event_types_TrafficScript%20Error (
  ...
  $basic__note                  = "Any problem that can be emitted by a TrafficScript/RuleBuilder rule.",
  ...
  info ("Configuring event_types_TrafficScript%20Error ${name}")
  vtmrest { "event_types/TrafficScript%20Error/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/event_types_TrafficScript%20Error.erb'),
    internal => 'event_types_TrafficScript%20Error',
    debug => 0,
  }
}
