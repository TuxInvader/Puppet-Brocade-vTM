# === Define: brocadevtm::monitors_Server%20First
#
...
define brocadevtm::monitors_Server%20First (
  ...
  info ("Configuring monitors_Server%20First ${name}")
  vtmrest { "monitors/Server%20First/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/monitors_Server%20First.erb'),
    internal => 'monitors_Server%20First',
    debug => 0,
  }
}
