# === Define: brocadevtm::event_types_SSL%20Certificate%20Expiry
#
...
define brocadevtm::event_types_SSL%20Certificate%20Expiry (
  ...
  info ("Configuring event_types_SSL%20Certificate%20Expiry ${name}")
  vtmrest { "event_types/SSL%20Certificate%20Expiry/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/event_types_SSL%20Certificate%20Expiry.erb'),
    internal => 'event_types_SSL%20Certificate%20Expiry',
    debug => 0,
  }
}
