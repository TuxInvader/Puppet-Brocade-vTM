# === Define: brocadevtm::monitors_Full%20HTTP
#
...
define brocadevtm::monitors_Full%20HTTP (
  ...
  info ("Configuring monitors_Full%20HTTP ${name}")
  vtmrest { "monitors/Full%20HTTP/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/monitors_Full%20HTTP.erb'),
    internal => 'monitors_Full%20HTTP',
    debug => 0,
  }
}
