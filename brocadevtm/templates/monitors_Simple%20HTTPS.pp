# === Define: brocadevtm::monitors_Simple%20HTTPS
#
...
define brocadevtm::monitors_Simple%20HTTPS (
  ...
  info ("Configuring monitors_Simple%20HTTPS ${name}")
  vtmrest { "monitors/Simple%20HTTPS/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/monitors_Simple%20HTTPS.erb'),
    internal => 'monitors_Simple%20HTTPS',
    debug => 0,
  }
}
