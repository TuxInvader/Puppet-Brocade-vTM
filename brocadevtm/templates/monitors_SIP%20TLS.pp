# === Define: brocadevtm::monitors_SIP%20TLS
#
...
define brocadevtm::monitors_SIP%20TLS (
  ...
  info ("Configuring monitors_SIP%20TLS ${name}")
  vtmrest { "monitors/SIP%20TLS/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/monitors_SIP%20TLS.erb'),
    internal => 'monitors_SIP%20TLS',
    debug => 0,
  }
}
