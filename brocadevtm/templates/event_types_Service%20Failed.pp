# === Define: brocadevtm::event_types_Service%20Failed
#
...
define brocadevtm::event_types_Service%20Failed (
  ...
  info ("Configuring event_types_Service%20Failed ${name}")
  vtmrest { "event_types/Service%20Failed/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/event_types_Service%20Failed.erb'),
    internal => 'event_types_Service%20Failed',
    debug => 0,
  }
}
