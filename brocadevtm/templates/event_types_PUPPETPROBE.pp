# === Define: brocadevtm::event_types_PUPPETPROBE
#
...
define brocadevtm::event_types_PUPPETPROBE (
  ...
  info ("Configuring event_types_PUPPETPROBE ${name}")
  vtmrest { "event_types/PUPPETPROBE/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/event_types_PUPPETPROBE.erb'),
    debug => 0,
  }
}
