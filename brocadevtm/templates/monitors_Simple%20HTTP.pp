# === Define: brocadevtm::monitors_Simple%20HTTP
#
...
define brocadevtm::monitors_Simple%20HTTP (
  ...
  info ("Configuring monitors_Simple%20HTTP ${name}")
  vtmrest { "monitors/Simple%20HTTP/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/monitors_Simple%20HTTP.erb'),
    internal => 'monitors_Simple%20HTTP',
    debug => 0,
  }
}
