# === Define: brocadevtm::monitors_Full%20HTTPS
#
...
define brocadevtm::monitors_Full%20HTTPS (
  ...
  info ("Configuring monitors_Full%20HTTPS ${name}")
  vtmrest { "monitors/Full%20HTTPS/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/monitors_Full%20HTTPS.erb'),
    internal => 'monitors_Full%20HTTPS',
    debug => 0,
  }
}
