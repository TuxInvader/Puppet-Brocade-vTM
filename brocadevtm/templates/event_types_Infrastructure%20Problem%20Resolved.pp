# === Define: brocadevtm::event_types_Infrastructure%20Problem%20Resolved
#
...
define brocadevtm::event_types_Infrastructure%20Problem%20Resolved (
  ...
  info ("Configuring event_types_Infrastructure%20Problem%20Resolved ${name}")
  vtmrest { "event_types/Infrastructure%20Problem%20Resolved/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/event_types_Infrastructure%20Problem%20Resolved.erb'),
    internal => 'event_types_Infrastructure%20Problem%20Resolved',
    debug => 0,
  }
}
