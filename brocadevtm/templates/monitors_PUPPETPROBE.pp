# === Define: brocadevtm::monitors_PUPPETPROBE
#
...
define brocadevtm::monitors_PUPPETPROBE (
  ...
  info ("Configuring monitors_PUPPETPROBE ${name}")
  vtmrest { "monitors/PUPPETPROBE/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/monitors_PUPPETPROBE.erb'),
    debug => 0,
  }
}
