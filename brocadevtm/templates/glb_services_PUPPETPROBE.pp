# === Define: brocadevtm::glb_services_PUPPETPROBE
#
...
define brocadevtm::glb_services_PUPPETPROBE (
  ...
  info ("Configuring glb_services_PUPPETPROBE ${name}")
  vtmrest { "glb_services/PUPPETPROBE/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/glb_services_PUPPETPROBE.erb'),
    debug => 0,
  }
}
