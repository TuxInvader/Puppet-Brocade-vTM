# === Define: brocadevtm::monitors_SIP%20UDP
#
...
define brocadevtm::monitors_SIP%20UDP (
  ...
  info ("Configuring monitors_SIP%20UDP ${name}")
  vtmrest { "monitors/SIP%20UDP/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/monitors_SIP%20UDP.erb'),
    internal => 'monitors_SIP%20UDP',
    debug => 0,
  }
}
