# === Define: brocadevtm::user_authenticators_PUPPETPROBE
#
...
define brocadevtm::user_authenticators_PUPPETPROBE (
  ...
  info ("Configuring user_authenticators_PUPPETPROBE ${name}")
  vtmrest { "user_authenticators/PUPPETPROBE/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/user_authenticators_PUPPETPROBE.erb'),
    debug => 0,
  }
}
