# === Define: brocadevtm::traffic_managers_07aff0d1e2f2
#

define brocadevtm::traffic_managers_07aff0d1e2f2 (
  $ensure,
  $basic__adminMasterXMLIP                = "172.17.42.4",
  $basic__adminSlaveXMLIP                 = "172.17.42.4",
  $basic__appliance_sysctl                = [],
  $basic__authenticationServerIP          = "172.17.42.4",
  $basic__cloud_platform                  = "",
  $basic__location                        = "",
  $basic__nameip                          = "",
  $basic__num_aptimizer_threads           = 0,
  $basic__num_children                    = 0,
  $basic__numberOfCPUs                    = 4,
  $basic__restServerPort                  = 11003,
  $basic__trafficip                       = [],
  $basic__updaterIP                       = "172.17.42.4",
  ...
  info ("Configuring traffic_managers_07aff0d1e2f2 ${name}")
  vtmrest { "traffic_managers/07aff0d1e2f2/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/traffic_managers_07aff0d1e2f2.erb'),
    internal => 'traffic_managers_07aff0d1e2f2',
    debug => 0,
  }
}
