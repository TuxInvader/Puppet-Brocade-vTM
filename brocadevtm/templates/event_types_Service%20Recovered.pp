# === Define: brocadevtm::event_types_Service%20Recovered
#
...
define brocadevtm::event_types_Service%20Recovered (
  ...
  info ("Configuring event_types_Service%20Recovered ${name}")
  vtmrest { "event_types/Service%20Recovered/${name}":
    endpoint => "https://${ip}:${port}/api/tm/3.3/config/active",
    ensure => $ensure,
    username => $user,
    password => $pass,
    type => 'application/json',
    content => template('brocadevtm/event_types_Service%20Recovered.erb'),
    internal => 'event_types_Service%20Recovered',
    debug => 0,
  }
}
